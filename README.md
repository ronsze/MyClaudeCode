...
Claude Code 플러그인. 코드 변경·병합·진단·리뷰·테스트 절차와 하네스 작성 규율을 프로젝트에 무관한 형태로 담는다. 프로젝트 고유 값은 각 프로젝트의 CLAUDE.md `## 프로젝트 사실` 절과 `.claude/` 오버레이 파일에 둔다.
...
| `skills/merge` | `/merge` — 브랜치 병합과 사전 검사·빌드·테스트 게이트. 프로젝트 고유 게이트는 `.claude/merge-gates.md` |
...
| `agents/design-matcher` | 디자인 시안 요소를 기존 UI 컴포넌트와 대조해 재사용·확장·신규 판정 |
...
| 파일 | 내용 | 형식 |
|---|---|---|
| `CLAUDE.md` `## 프로젝트 사실` | 기본 브랜치, 컴파일·빌드·테스트 명령, 커밋 제목 형식, 계약 정의, 스펙 위치 등 | `skills/harness/references/claude-md.md`의 항목 표 |
