...
- **훅 하나에 목적 하나**: 한 줄을 넘는 로직은 `.claude/hooks/<name>.sh|py` 스크립트로 분리하고, 스크립트 최상단 주석에 목적을 서술한다(제1원칙 — JSON에는 주석을 둘 수 없다). settings.json에는 `"$CLAUDE_PROJECT_DIR/.claude/hooks/<name>.py"`처럼 절대 경로로 참조한다.
