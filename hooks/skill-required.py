...
#   판정: 세션 트랜스크립트에서 Skill 도구 호출 또는 `/<이름>` 슬래시 호출을 찾는다. 플러그인 스킬의
#   `<플러그인>:<이름>` 형태도 같은 이름으로 본다.
...
def matches(name, skills):
    # `trello`와 `my-claude-code:trello`를 같은 스킬로 본다.
    return isinstance(name, str) and name.split(":")[-1] in skills


def is_loaded(transcript_path, skills):
    slash = re.compile(r"(?:<command-name>|/)(?:[A-Za-z0-9_-]+:)?(%s)\b" % "|".join(re.escape(s) for s in skills))
    with open(transcript_path, encoding="utf-8") as f:
        for line in f:
            for block in iter_blocks(line):
                if block.get("type") == "tool_use" and block.get("name") == "Skill":
                    if matches((block.get("input") or {}).get("skill"), skills):
                        return True
                if block.get("type") == "text":
                    if slash.search(block.get("text") or ""):
                        return True
    return False
