...
# 상태 파일: 프로젝트의 `.claude/model-review.txt`, `<계열> <모델ID> <pending|reviewed>` 한 줄씩. 재검토 상태는 프로젝트별이다.
#   프로젝트 루트는 CLAUDE_PROJECT_DIR, 없으면 stdin의 cwd, 없으면 현재 디렉토리.
# 검증: model-review.txt를 백업한 뒤 `echo '{}' | CLAUDE_PROJECT_DIR=<프로젝트> ANTHROPIC_DEFAULT_<계열>_MODEL=<바뀐 모델ID> hooks/model-review.py`로
...
STATE = None  # main()에서 프로젝트 루트가 정해진 뒤 채운다.
AGENTS = None
...
def main():
    ...
    STATE = os.path.join(project, ".claude", "model-review.txt")
    AGENTS = os.path.join(project, ".claude", "agents", "*.md")
